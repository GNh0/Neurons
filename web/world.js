...
export function setupWorld({api,toast,showPanel,observe}){
  ...
  function render(){
    ...
    $('worldModelNote').textContent=`주변 지형 ${state.world.cached_chunks}/${state.world.chunk_limit} 청크 · ${state.model.action_neurons_per_agent}개 행동 뉴런 · ${fmt(state.model.readout_synapses_per_agent)}개 학습 시냅스 / 개체 · 수명은 세계 턴 기준`;
  ...
  function renderHistory(){
    $('individualHistory').innerHTML=[...(detail.learning_records||[]).slice(0,8),...(detail.events||[]).filter(e=>e.kind==='experience').slice(0,12)].map(e=>{
      if(e.kind==='experience')return `<div class="individual-event"><small>행동 ${e.step} · ${esc(actionName(e.action))}</small><br>${esc(e.observation||(e.collision?'충돌':e.new_place?'새 지역':'이동'))}<br>보상 ${e.reward.toFixed(2)} · 가중치 Δ ${e.weight_change.toFixed(3)}</div>`;
